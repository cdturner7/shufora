...
const DEFAULTS: AppearanceState = { theme: 'system', accentColor: 'amber' };
...
export function AppearanceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists()) {
        ...
        applyAccent(remote.accentColor);
        setState(remote);
  ...
  useEffect(() => {
    ...
    function onSystemChange() { if (state.theme === 'system') applyTheme('system'); }
  ...
  const setTheme = useCallback((theme: Theme) => {
    setState((prev) => { const next = { ...prev, theme }; applyTheme(theme); persist(next); return next; });
  }, [user?.uid]); // eslint-disable-line react-hooks/exhaustive-deps

  const setAccentColor = useCallback((accentColor: AccentColor) => {
    setState((prev) => { const next = { ...prev, accentColor }; applyAccent(accentColor); persist(next); return next; });
  }, [user?.uid]); // eslint-disable-line react-hooks/exhaustive-deps

  return (
    <AppearanceContext.Provider value={{ ...state, setTheme, setAccentColor, naming: SHUFORA_NAMING }}>
      {children}
